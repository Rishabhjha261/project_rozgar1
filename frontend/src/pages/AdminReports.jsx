...
export default function AdminReports() {
  ...
  useEffect(() => {
    ...
    async function run() {
      const ids = Array.from(new Set((reports || []).map((r) => r.jobId))).filter(Boolean)
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <p className="mt-2 text-sm text-slate-600">Review reports submitted by users.</p>
      ...
      <div className="mt-4 grid gap-3">
        {reportsStatus === 'loading' ? (
          ...
          <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">
            Failed to load reports. {reportsError?.message ? `(${reportsError.message})` : ''}
          </div>
        ) : filtered.length ? (
          filtered.map((r) => {
              ...
              <div key={r.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-slate-500">{formatTime(r.createdAt)}</div>
                  </div>
                ...
                <div className="mt-3 text-sm text-slate-700">
                  <span className="font-semibold">Reason:</span> {t(r.reason) || r.reason}
                </div>
                {r.notes ? <div className="mt-2 text-sm text-slate-600">Notes: {r.notes}</div> : null}

                {job ? (
                  <div className="mt-4 rounded-2xl bg-slate-50 p-4">
                    <div className="text-sm font-semibold text-slate-900">Job: {job.title}</div>
                    <div className="mt-1 text-xs text-slate-600">{job.area}</div>
                    <Link
                    ...
                    {isHidden ? (
                      <div className="mt-2 text-xs font-semibold text-rose-600">Currently hidden from users</div>
                    ) : null}
                  </div>
                ) : (
                  <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">Job not found.</div>
                )}
              </div>
            )
          })
